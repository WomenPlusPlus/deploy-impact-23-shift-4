    ...
    def match_jobs():
        ...
        try:
            if request.method == "POST":
                ...
                candidate = requests.post('http://localhost:5001/api/get_candidate_by_id', json=cand_json)
                cand_skills = { skill['skill_name']:skill['skill_level'] for skill in candidate.json()['candidates']['skills'] }
                    ...
                    if job_skills:
                        ...
                        if job_score > 70:
                            ...
                            if job['matching_candidates']:
                                job['matching_candidates'].append({'id':id, 'score':job_score})
                            else:
                                job['matching_candidates'] = []
                                job['matching_candidates'].append({'id':id, 'score':job_score})
                            update_job_json = {"job_id":job_id, 'matching_candidates': job['matching_candidates']}
                            update_job_response = requests.put('http://localhost:5001/api/update_job', json=update_job_json)
                
                update_json = {'user_id': id, 'matching_jobs': job_match}
                update_cand = requests.put('http://localhost:5001/api/update_candidate', json=update_json)

                if update_cand.status_code == 200:
                    return jsonify({'message':'Update Candidate successfully', 'matching_jobs': job_match}), 200
