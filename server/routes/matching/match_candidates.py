    ...
    def match_candidates():
        ...
        try:
            if request.method == "POST":
                ...
                job_skills = [skill['skill_name'] for skill in job.json()['jobs']['skills']]
                ...
                for candidate in candidates:
                    cand_skills = { skill['skill_name']:skill['skill_level'] for skill in candidate['skills'] }
                    ...
                    if cand_skills:
                        ...
                        if cand_score > 70:
                            ...
                            if candidate['matching_jobs']:
                                candidate['matching_jobs'].append({ 'id':cand_id, 'score': cand_score })
                            else:
                                candidate['matching_jobs'] = []
                                candidate['matching_jobs'].append({ 'id':cand_id, 'score': cand_score })
                            update_cand_json = {"user_id":cand_id, 'matching_jobs': candidate['matching_jobs']}
